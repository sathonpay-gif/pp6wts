...
async function renderScores(p){
  ...
  p.innerHTML=`<h2>กรอกคะแนนรายวิชา</h2><div class="card"><div class="toolbar"><select id="assignSelect" onchange="loadScoreGrid()"><option value="">-- เลือกวิชา / ห้อง --</option>${assigns.map(a=>`<option value="${a.assignmentId}">${escapeHtml(a.subjectCode+' '+a.subjectName+' · '+a.className)}</option>`).join('')}</select><button class="btn btn-secondary" onclick="openPasteScoreModal()">📋 วางคะแนนจาก Excel</button><button id="saveScoreBtn" class="btn btn-primary" onclick="saveScoreGrid()" disabled>บันทึกคะแนน</button></div><div id="scoreInfo"></div><div id="scoreGrid" class="table-wrap"></div></div>`;
...
async function renderSubjects(p){
  ...
  <div class="card curriculum-card"><div class="section-heading"><div><span class="section-kicker">CURRICULUM</span><h3>รายวิชา ภาคเรียนที่ 1</h3></div><span class="soft-badge">ม.1 · ม.2 · ม.3</span></div><div class="curriculum-grid">${term1}</div></div>
  <div class="card curriculum-card"><div class="section-heading"><div><span class="section-kicker">CURRICULUM</span><h3>รายวิชา ภาคเรียนที่ 2</h3></div><span class="soft-badge">ม.1 · ม.2 · ม.3</span></div><div class="curriculum-grid">${term2}</div></div>
...
function renderPP6PageHtml(d){
  ...
  return `<div class="pp6-page"><style>
    ...
    .tbl th,.tbl td{border:1px solid #000;padding:0.47mm 3px;line-height:1;font-size:14pt;text-align:center;vertical-align:middle}
...
async function renderSummary(p){
  ...
  p.innerHTML=`<h2>อันดับผลการเรียน (GPA)</h2><div class="card"><div class="toolbar"><select id="sumClass" onchange="loadSummary()"><option value="">-- เลือกห้อง --</option>${cls.map(c=>`<option value="${c.classId}">${escapeHtml(c.className)}</option>`).join('')}</select></div><div id="summaryOut"></div></div>`;
}
...
async function savePeriodSettingNow(setCurrent){try{const year=$('newPeriodYear').value.trim(),term=$('newPeriodTerm').value,label=$('newPeriodLabel').value.trim();if(!year||!term)return toast('กรุณาระบุปีการศึกษาและภาคเรียน');if(setCurrent){await call('setCurrentAcademicPeriod',state.token,year,term);}else{await call('saveAcademicPeriod',state.token,{year,term,label});}state.periods=await call('getAcademicPeriods',state.token);if(setCurrent){state.period={year,term};localStorage.setItem('pp6_period',JSON.stringify(state.period));state.dashboard=await call('getDashboard',state.token,year,term);}toast(setCurrent?'ตั้งเป็นช่วงปัจจุบันแล้ว':'บันทึกช่วงการศึกษาแล้ว',true);await renderUsers($('page'));}catch(e){toast(e.message);}}
